...
def config():
    ...
    if 'src.relayer.config' in sys.modules:
        importlib.reload(sys.modules['src.relayer.config'])
    import src.relayer.config as config
    return config
...
class TestConfig:
    ...
    def test_get_blockchain_config_returns_dto_with_chain_id(
        self, 
        config,
    ):
    ...
    def test_get_blockchain_config_raise_exception_with_bad_chain_id(
        self, 
        config,
    ):
        """
    ...
    def test_get_register_config_returns_dto_with(
        self, 
        config,
    ):
    ...
    def test_get_register_config_raise_exception_with_data_missing(
        self, 
        config,
    ):
        """
        ...
        arguments.
        """
        with patch('src.relayer.config._get_bridge_relayer_config'):
            with pytest.raises(BridgeRelayerConfigRegisterDataMissing):
                config.get_register_config()
            
        
    def test_bridge_relayer_config_envi_is_dev(
        self, 
        config
    ):
        """Test bridge_relayer_config is set to dev env."""
        assert config.is_dev_env() == True

    # ----------------------------------------------------------
    # Internal fonctions
    # ----------------------------------------------------------
    def test_load_env_file_with_valid_file(self, config):
        """
    ...
    def test_get_toml_file_returns_prod_env(
        self, 
        config_prod
    ):
    ...
    def test_get_config_content_raise_exception_toml_file_missing(
        self, config
    ):
        """
    ...
    def test_get_abi_file_raises_exception_file_missing(
        ...
        """
        with patch('src.relayer.config.get_abi_file', return_value="missing_abi_file"):
            with pytest.raises(BridgeRelayerConfigABIFileMissing):
                config.get_abi(chain_id=80002)             
        
    def test_get_abi_returns_valid_abi(self, config):
        """Test get_abi that returns an abi for a specific chain_id"""
        abi = config.get_abi(chain_id=80002)
        assert len(abi) > 0
        
    def test_get_abi_raise_exception_with_invalid_chain_id(
        self, 
        config
    ):
        """
    ...
    def test_get_abi_file_returns_valid_file_for_prod(
        self, 
        config_prod
    ):
