...
def test_result_dto_can_be_set_with_any_types(types, expected):
...
def test_equality_between_two_instances():
...
def test_inequality_between_two_instances():
...
def test_base_result_as_bool_is_true():
    """Test BaseResult ok is set as True then as bool is True."""
    result = BaseResult()
    result.ok = 123
    assert bool(result) is True
    
def test_base_result_as_bool_is_false():
